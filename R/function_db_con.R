...
#' 
#' Wrapper around RMariaDB::dbConnect to connect to itembank, checkauswertung (sessions / results), dateneingabe, and check_data_revisions (mutterdatei). An active SSH tunnel is required!
#' 
...
db_con <- function(user, db = c("all", "itembank", "dateneingabe", "checkauswertung", "mutterdatei", "tracking", "check_data_revisions") ) {
  ...
  db <- unique(db)
  if (any(db == "all")) db <- c("itembank", "dateneingabe", "checkauswertung", "check_data_revisions")
  pw <- rstudioapi::askForPassword(paste("DB server password for", user))
  cons <- list()
  for (dbi in db) {
    hostname <- switch(dbi,
                       itembank = "127.0.0.1",
                       dateneingabe = "127.0.0.1",
