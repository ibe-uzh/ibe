#' Get calibration-input files (input into TAM scripts) from IBE server
#'
...
#'
#' @examples
#' get_calib_input_file("S2", 2022, "dles", read_data = FALSE)
#' head(get_calib_input_file("S2", 2022, "dles"))
#'
#' @export
get_calib_input_file <- function(check, year, domain,
                                 ...
                                 hybrid = NULL)
{
  ...
  if (check_type == "P") {
    ...
    if (is.null(spec_regex)) {
      if (is_schreib) {
        ...
        file_rx <- paste0(domain,"_(P|p)",substr(check,2,2),"_","[[:digit:]]{2}",year_short,"_calib")
        if (check=="P5" & year >= 2023 & hybrid == "paper") file_rx <- sub("calib", "?(paper)_calib", file_rx) # select paper vs online calib input file for hybrid P5-checks
        if (check=="P5" & year >= 2023 & hybrid == "online") file_rx <- sub("calib", "CALIB", file_rx)
      }
    ...
    if (is.null(spec_regex)) {
      if (is_schreib) {
        ...
        file_rx <- paste0(domain,"_",check,"_",year,"_(CALIB|calib)")
      }
    } else file_rx <- spec_regex
  ...
  if (nrow(files) == 0) {
    message("ERROR: No CALIB file found for Check ",check," ",year," in ",domain,
            "\nPath: ",dirpath,
